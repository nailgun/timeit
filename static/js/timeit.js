window.timeit = (function() {
    ...
    timeit.utils.template = function (name, callback) {
        var defaultContextExtensions = {
            dom: [timeit.utils.contextExtensions.include],
            plain: []
        };
        ...
        if (name.slice(-5) == '.html') {
            contextExtensions = defaultContextExtensions.dom;
            return domTemplate(name, wrapTemplate);
        } else {
            contextExtensions = defaultContextExtensions.plain;
    ...
    timeit.utils.contextExtensions = {};

    timeit.utils.contextExtensions.include = function (context) {
        ...
        context.include = function(name) {
            ...
            context._done.push(function($dom) {
                timeit.utils.template(name, function (template) {
    ...
    timeit.utils.TemplateMixin = {
        render: function () {
           ...
           timeit.utils.template(this.template, function(template) {
                var html = null;
